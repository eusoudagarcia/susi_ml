...
// print('zeros shape ${zeros.shape}');
  ...
  List<int> getPatchIndices(List<int> kernelSize, List<int> shape) {
    List<int> indices = [];

    int start0 = (kernelSize[0] - 1) ~/ 2;
    int end0 = shape[0] - (kernelSize[0] - 1);

    int start1 = (kernelSize[1] - 1) ~/ 2;
    int end1 = shape[1] - (kernelSize[1] - 1);

    int start2 = 0; // Início para a terceira dimensão (canais de cores)
    int end2 = shape[2]; // Fim para a terceira dimensão

    for (int i = start0; i < end0; i += kernelSize[0]) {
      for (int j = start1; j < end1; j += kernelSize[1]) {
        for (int k = start2; k < end2; k++) {
          indices.add((i * shape[1] + j) * shape[2] + k);
        }
      }
    }
  ...
  List<Tensor> patches(
    ...
    for (int rowIndex in indices) {
      Tensor row = getRows([rowIndex], shape, _data);

      int start0 = rowIndex * kernelSize[0];
      int end0 = start0 + kernelSize[0];
      int start1 = rowIndex * kernelSize[1];
      int end1 = start1 + kernelSize[1];
      int start2 = rowIndex * kernelSize[2];
      int end2 = start2 + kernelSize[2];

      if (end0 > _shape[0] || end1 > _shape[1] || end2 > _shape[2]) {
        break;
      }

      Tensor patch =
          row.slice1(start0, end0, strides, start1, end1, row.data, row.shape);

      patches.add(patch);
      print('patches length ${patches.length}');
    }
  ...
  Tensor getRows(List<int> indices, List<int> shape, List<double> data) {
    int height = shape[0];
    int width = shape[1];
    int channels = shape[2];

    int totalSize = height * width * channels;
    List<double> selectedRows = [];

    for (int index in indices) {
      if (index >= 0 && index < totalSize) {
        int start = index * channels;
        int end = start + channels;

        selectedRows.addAll(data.sublist(start, end));
      }
    }

    return Tensor.createTensor(selectedRows,
        shape: [indices.length, width, channels]);
  }
  ...
  //   return newTensor;
  // }
  Tensor slice1(int start0, int end0, List<int> strides, int start1, int end1,
      List<double> data, List<int> shape) {
    int height = shape[0];
    int width = shape[1];
    int channels = shape[2];
    ...
    if (start0 < 0 || end0 > totalSize || end1 > totalSize) {
      throw Exception('Invalid slice indices');
    }

    int rows = (end0 - start0) ~/ channels;
    int cols = end1 - start1;

    List<double> slicedData = [];
    for (int i = start0; i < end0; i += channels) {
      int rowStart = i * width * channels;
      int rowEnd = rowStart + width * channels;

      for (int j = start1; j < end1; j++) {
        slicedData.add(data[rowStart + j]);
      }
    }

    return Tensor.createTensor(slicedData, shape: [rows, cols]);
  }
...
// Get the indices of the patches
    List<int> patchIndices = getPatchIndices(kernelSize, shape);
...
// Split the input into patches
    ...
    for (int i = 0; i < patches.length - 1; i++) {
      for (int j = 0; j < patches[i].size; j++) {
        int index = i * patches[i].size + j;
        if (index >= output.size) {
          break;
        }
        output.setValue(index, patches[i]._data[j]);
      }
    }
...
//   }
  ...
  List<int> _calculateStrides(List<int> shape) {
    ...
    // Calcula os strides para lidar com as dimensões (altura, largura, canais de cores)
