...
// print('zeros shape ${zeros.shape}');
  ...
  List<Tensor> patches(
    ...
    for (int rowIndex in indices) {
      ...
      // Tensor patch = row.slice(0, kernelSize[0], strides, kernelSize[0] + row, end1);
  ...
  Tensor getRow(int index) {
    int totalElementsPerRow = _shape[1] * _shape[2];
    Tensor rowTensor = Tensor.zeros([totalElementsPerRow]);

    int startingIndex = index * totalElementsPerRow;
    int endingIndex = startingIndex + totalElementsPerRow;

    if (index >= 0 && index < _shape[0] * _shape[1] * _shape[2]) {
      for (int i = 0; i < totalElementsPerRow; i++) {
        if (startingIndex == endingIndex) {
          break;
        }
        rowTensor.setValue(i, _data[startingIndex]);
        startingIndex++;
      }
    }
    return rowTensor;
...
// Calculate the start and end indices of the patches
    print('calculating start0');
    int start0 = (kernelSize[0] - 1) ~/ 2;
    print('calculating end0');
...
// print('i + outputShapeOrder * i ${i + outputShapeOrder * i}');
      if (i >= totalOutputElements || i + outputShapeOrder * i > output.size) {
        print('break at $i ');
